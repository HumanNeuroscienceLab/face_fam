...
convolve.spm.hrf1 <- function(onsets, durs, tot.time, runs, 
                             mags=1, frate=1/24, tr=1, parallel=F)
{
  s1 <- gen.events(onsets, durs, tot.time, mags, frate)
  c1 <- apply.convolve(s1, runs, hrf_fun=spmt, frate=frate, parallel=parallel)
  d1 <- downsample(c1, tr, frate)
  return(d1)
}

convolve.spm.hrf2 <- function(onsets, durs, tot.time, runs, 
                              mags=1, frate=1/24, tr=1, parallel=F)
{
  s1 <- gen.events(onsets, durs, tot.time, mags, frate)
  c1 <- apply.convolve(s1, runs, hrf_fun=dspmt, frate=frate, parallel=parallel)
  d1 <- downsample(c1, tr, frate)
  return(d1)
}
...
# Do the regular one (double gamma HRF)
sub.bs1 <- laply(subjects, function(subj) {
  ...
  system.time(vid.bs <- laply(vnames, function(vname) {
    ...
    # Run regression
    dmat <- model.matrix(~cur.trial + other.trials + quests + days + repeat.faces + mc)
...
sub.bs2 <- laply(subjects, function(subj) {
  ...
  onsets3 <- vid.timing$onset[vid.timing$question!="none"]
  durs3   <- 4
  quests  <- convolve.spm.hrf1(onsets3, durs3, tot.time, runs, frate=1/12)
  ...
  onsets4    <- vid.timing$onset[repeat.day==2]
  durs4      <- vid.timing$duration[repeat.day==2]
  repeat.faces <- convolve.spm.hrf1(onsets4, durs4, tot.time, runs, frate=1/12)
  ...
  system.time(vid.bs <- laply(vnames, function(vname) {
    ...
    onsets1   <- vid.timing$onset[vid.timing$video == vname]
    durs1     <- vid.timing$duration[vid.timing$video == vname]
    cur.trial <- convolve.spm.hrf1(onsets1, durs1, tot.time, runs, frate=1/12)
    
    # All other trials
    onsets2 <- vid.timing$onset[vid.timing$video != vname]
    durs2   <- vid.timing$duration[vid.timing$video != vname]
    other.trials <- convolve.spm.hrf1(onsets2, durs2, tot.time, runs, frate=1/12)
...
# SPM HRF2
sub.bs3a <- laply(subjects, function(subj) {
  dat <- dat.vols[[subj]]
  ...
  system.time(vid.bs <- laply(vnames, function(vname) {
    ...
    onsets1   <- vid.timing$onset[vid.timing$video == vname]
    durs1     <- vid.timing$duration[vid.timing$video == vname]
    cur.trial <- convolve.spm.hrf(onsets1, durs1, tot.time, runs, nparams=2, frate=1/12)
    
    # All other trials
    onsets2 <- vid.timing$onset[vid.timing$video != vname]
    durs2   <- vid.timing$duration[vid.timing$video != vname]
    other.trials <- convolve.spm.hrf(onsets2, durs2, tot.time, runs, nparams=2, frate=1/12)
...
sub.bs3b <- laply(subjects, function(subj) {
  dat <- dat.vols[[subj]]
...
save(sub.bs1, sub.bs2, sub.bs3a, sub.bs3b, 
     file="/data1/famface01/analysis/misc/320_roi_task_activity/std_averoi_betas.rda")

# This is code to check if my convolution approach is the same as the one for 
# AFNI
...
my.xmat <- laply(vnames, function(vname) {
  # Current trial
  onsets1   <- vid.timing$onset[vid.timing$video == vname]
  durs1     <- vid.timing$duration[vid.timing$video == vname]
  cur.trial <- convolve.spm.hrf(onsets1, durs1, tot.time, runs, nparams=1, frate=1/12)[,1]
  
  # All other trials
  onsets2 <- vid.timing$onset[vid.timing$video != vname]
  durs2   <- vid.timing$duration[vid.timing$video != vname]
  other.trials <- convolve.spm.hrf(onsets2, durs2, tot.time, runs, nparams=1, frate=1/12)[,1]
  
  cbind(cur=cur.trial, other=other.trials)
}, .progress="text")
...
curdir <- getwd()
setwd(td)
afni.xmat <- laply(vnames, function(vname) {
  onsets1 <- sapply(unique(runs), function(run) {
    select.inds <- (vid.timing$video == vname) & (vid.timing$run==run)
    if (sum(select.inds) == 0) {
      ret <- "*"
    } else {
      ret <- paste(round(vid.timing$onset[select.inds], 5), collapse=" ")
    }
    ret
  })
  onsets2 <- sapply(unique(runs), function(run) {
    select.inds <- (vid.timing$video != vname) & (vid.timing$run==run)
    if (sum(select.inds) == 0) {
      ret <- "*"
    } else {
  ...
  writeLines(onsets1, con = "tmp_onsets1.txt")
  writeLines(onsets2, con = "tmp_onsets2.txt")
  
  ntpts <- 318*length(unique(runs))
  cmd <- "3dDeconvolve -global_times -x1D_stop -polort -1 -nodata %i 1 -num_stimts 2 -stim_times 1 tmp_onsets1.txt 'SPMG1' -stim_label 1 curtrial -stim_times 2 tmp_onsets2.txt 'SPMG1' -stim_label 2 othertrial -x1D tmp"
  cmd <- sprintf(cmd, ntpts)
  system(cmd, ignore.stdout = T, ignore.stderr = T)
  
  xmat <- read.table("tmp.xmat.1D")
  colnames(xmat) <- c("cur", "other")
  
  as.matrix(xmat)
}, .progress="text")
setwd(curdir)

my.results <- aaply(my.xmat, .(1), function(x) {
  cur.trial <- x[,1]
  other.trials <- x[,2]
  mc <- load.mc(subj)
  dmat <- model.matrix(~cur.trial + other.trials + mc)
  ret.fit <- simple_lm(rdats, dmat)
...
afni.results <- aaply(afni.xmat, .(1), function(x) {
  cur.trial <- x[,1]
  other.trials <- x[,2]
  mc <- load.mc(subj)
  dmat <- model.matrix(~cur.trial + other.trials + mc)
...
cor(cur.trial, tmp[,1])
cor(other.trials, tmp[,2])

# scale
plot.ts(cbind(other.trials, tmp[,2]*2.2)[1:200,], plot.type="single", col=1:2)

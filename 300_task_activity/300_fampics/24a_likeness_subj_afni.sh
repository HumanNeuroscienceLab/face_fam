...
# parallel --no-notice -j $njobs --eta bash 24a_likeness_subj_afni.sh {} ::: ${subjects[@]}
...
outdir="${outbase}/fampics_likeness.reml"
...
if [[ "$subj" -eq "sub01"  ]]; then
  ${tog}/task_analysis.rb -i ${fundir}/fam_pics/filtered_func_run??.nii.gz \
    ...
    --stim-am1 likeness ${tdir}/stimam_likeness_group.txt 'SPMG1(1)' \
    --regdir ${fundir}/reg \
    --tostandard \
    --threads ${nthreads}

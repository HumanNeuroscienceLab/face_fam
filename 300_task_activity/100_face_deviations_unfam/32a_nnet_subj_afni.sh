...
# parallel --no-notice -j $njobs --eta bash 32a_nnet_subj_afni.sh {} ::: ${subjects[@]}
...
outdir="${outbase}/nnet_mean_diff.reml"
...
${tog}/task_analysis.rb -i ${fundir}/unfam_vids/filtered_func_run??.nii.gz \
  ...
  --stim-am1 mean_diff ${tdir}/stimam_nnet_mean_diff.txt 'SPMG1(2)' \
  --stim-am1 mean_framediff ${tdir}/stimam_nnet_mean_frame_diff.txt 'SPMG1(2)' \

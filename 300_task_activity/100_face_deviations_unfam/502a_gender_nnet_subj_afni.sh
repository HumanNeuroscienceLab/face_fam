...
# parallel --no-notice -j $njobs --eta bash 502a_gender_nnet_subj_afni.sh {} ::: ${subjects[@]}
...
outdir="${outbase}/nnet_masked_gender_diff.reml"
...
${tog}/task_analysis.rb -i ${fundir}/unfam_vids/filtered_func_run??.nii.gz \
  ...
  --stim-am1 diff_male ${tdir}/stimam_nnet_masked_mean_diff_male.txt 'SPMG1(2)' \
  --stim-am1 diff_female ${tdir}/stimam_nnet_masked_mean_diff_female.txt 'SPMG1(2)' \
  --stim-am1 gender ${tdir}/stimam_gender_diff.txt 'SPMG1(2)' \
  --stim quests ${tdir}/stim_questions.txt 'SPMG1(4)' \
  --glt diff_gender 'SYM: +0.5*diff_male +0.5*diff_female' \
  --regdir ${fundir}/reg \
  --tostandard \
  --threads ${nthreads}

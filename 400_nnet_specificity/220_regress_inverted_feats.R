...
indir <- "/data1/famface01/analysis/misc/openface/layer_features_inverted"
...
## inverted features
reps <- read.csv("/data1/famface01/analysis/misc/openface/unfam_flipped/reps.csv", header=F)
labs <- read.csv("/data1/famface01/analysis/misc/openface/unfam_flipped/labels.csv", header=F)
labs$fn <- sub("png", "mat", basename(as.character(labs$V2)))
oinds <- sapply(as.character(vdf2$fn), function(fn) {
  which(fn == labs$fn)
})
all.equal(as.character(labs$fn)[oinds], as.character(vdf2$fn))
reps2 <- reps[oinds,]
...
nlayers  <- 26
bdir <- "/data1/famface01/analysis/misc/openface/concatenate_layers_inverted"
